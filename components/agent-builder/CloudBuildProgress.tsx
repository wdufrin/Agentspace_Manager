...
const CloudBuildProgress: React.FC<CloudBuildProgressProps> = ({ projectId, buildId, onClose }) => {
    ...
    return (
        <div 
            ...
            className={`fixed bottom-4 right-4 z-50 bg-gray-800 rounded-full shadow-lg border ${getBorderColor()} p-1 pl-4 pr-2 flex items-center gap-3 cursor-pointer hover:bg-gray-700 transition-all`}
        >
            <div className="flex items-center gap-2">
                {!isFinished ? (
                ...
                <span className="text-sm font-medium text-white">
                    {!isFinished ? 'Deploying Agent...' : isSuccess ? 'Deployment Successful' : 'Deployment Failed'}
                </span>
            </div>
            <button 
