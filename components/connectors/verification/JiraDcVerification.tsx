...
interface JiraDcVerificationProps {
    dataMode: DataMode;
}

const JiraDcVerification: React.FC<JiraDcVerificationProps> = ({ dataMode }) => {
        ...
        <div className="space-y-6 animate-fadeIn">
            <div>
                <h3 className="text-sm font-bold text-gray-300 mb-4 uppercase tracking-wider">
                     {dataMode === 'INGESTION' ? 'Jira Data Center Ingestion Setup' : 'Jira Data Center Federated Setup'}
                </h3>
                ...
                <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 space-y-8">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                             ...
                             />
                             <ChecklistItem
                                id="jira_dc_admin_group"
                                label="Add to Admin Group"
                                subLabel="Go to Groups. Add the new user to the 'jira-administrators' group."
                                badge="Required"
                             />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mb-4">
                            Unlike Jira Cloud which uses a global app, Jira Data Center requires an incoming External Application link configured directly on your instance.
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <ChecklistItem
                                    id="jira_dc_app_link"
                                    label="Create Link"
                                    subLabel="Go to Jira Administration > Applications > Application links. Click 'Create link'."
                                />
                                <ChecklistItem
                                    id="jira_dc_app_type"
                            ...
                            <div>
                                ...
                                {dataMode === 'INGESTION' ? (
                                    <div className="bg-gray-900/50 border className-gray-700 p-3 rounded">
                                        <p className="text-sm font-medium text-white mb-1">Data Ingestion</p>
                                        <p className="text-xs text-gray-400">Select <strong className="text-blue-400">Read</strong> permission.</p>
                                        <p className="text-[10px] text-gray-500 mt-2">Required to ingest projects, comments, issues, attachments, and worklogs.</p>
                                    </div>
                                ) : (
                                    <div className="bg-gray-900/50 border border-gray-700 p-3 rounded">
                                        <p className="text-sm font-medium text-white mb-1">Federated Search</p>
                                        <p className="text-xs text-gray-400">Select <strong className="text-purple-400">Write</strong> permission.</p>
                                        <p className="text-[10px] text-gray-500 mt-2">Required to perform search and optionally update issues (action scopes). Write implies Read.</p>
                                    </div>
                                )}
                            </div>
                        </div>
...
export default JiraDcVerification;
