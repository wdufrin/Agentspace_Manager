    ...
    return (
        ...
        >
            ...
            <p className="text-sm text-gray-400 mb-4 flex-grow">
                Python agent sample from GitHub. Click to view architecture and deploy.
            </p>
...
const AgentCatalogPage: React.FC<AgentCatalogPageProps> = ({ projectNumber, setProjectNumber, accessToken }) => {
  ...
  const fetchGitAgents = useCallback(async () => {
      ...
      try {
          ...
          // Default branch is usually main or master if not specified, but tree url specifies it
  ...
  const fetchRepoContents = async (url: string, prefix: string = '', depth: number = 0): Promise<{name: string, content: string}[]> => {
      // Safety limit to prevent infinite recursion or massive fetches
      if (depth > 5) return [];
      ...
      for (const item of items) {
          if (item.type === 'file' && item.download_url) {
              setLoadingAgentFiles(`Fetching ${prefix}${item.name}`); // Update UI status
              ...
              if (fileRes.ok) {
                  ...
                  // Don't throw here, partial load is better than nothing
              }
          } else if (item.type === 'dir') {
              // Recursive call for subdirectories
              // Use the item.url which points to the API endpoint for this directory
  ...
  const filteredGitAgents = useMemo(() => {
      if (!searchTerm) return gitAgents;
      const lowerTerm = searchTerm.toLowerCase();
      return gitAgents.filter(a => a.name.toLowerCase().includes(lowerTerm));
  }, [gitAgents, searchTerm]);
                ...
                >
                 ...
                 <div className="w-1/3">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            className="block w-full pl-10 pr-3 py-2 border border-gray-600 rounded-md leading-5 bg-gray-700 text-gray-300 placeholder-gray-400 focus:outline-none focus:bg-gray-600 focus:ring-1 focus:ring-teal-500 focus:border-teal-500 sm:text-sm h-[42px]"
